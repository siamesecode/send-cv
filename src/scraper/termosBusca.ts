// Lista de termos de busca para empresas de tecnologia
...
export function gerarTermosBusca(cidade?: string): string[] {
  if (cidade) {
    return termosBuscaTecnologia.map(termo => `${termo} em ${cidade}`);
  }

  return termosBuscaTecnologia.map(termo => `${termo} Brasil`);
}

export function gerarTodasBuscas(): string[] {
  const buscas: string[] = [];
  
  for (const cidade of cidadesPrincipais) {
    const termosCidade = gerarTermosBusca(cidade);
    buscas.push(...termosCidade.slice(0, 3)); 
  }
